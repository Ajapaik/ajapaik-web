...
def add_subject(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_album.open = True
            new_album.profile = request.user.profile if request.user.is_authenticated else None
            new_album.save()
...
def get_subject_data(request: HttpRequest, rectangle_id=None):
    print(FaceRecognitionRectangle.objects.all().values_list(
        'face_recognition_suggestion_subject_datas',
        flat=True))
    print(FaceRecognitionRectangle.objects.annotate(
        number_of_suggestions=Count('face_recognition_suggestions')).order_by(
        '-number_of_suggestions').first().number_of_suggestions)
    ...
    if album_id:
        ...
        unverified_rectangles = unverified_rectangles.filter(photo_id__in=album_photo_ids)
        rectangle = FaceRecognitionRectangle.objects.filter(photo_id__in=album_photo_ids).order_by('?').first()
    else:
    ...
    if unverified_rectangles.count() > 1:
        ...
        if profile:
            suggestions = FaceRecognitionRectangleSubjectDataSuggestion.objects.filter(
                proposer_id=profile.id).all().values_list(
                'face_recognition_rectangle_id', flat=True)
            rectangles = rectangles.exclude(id__in=suggestions)

        if album_id:
            rectangles = rectangles.filter(photo_id__in=album_photo_ids)

        if not rectangles:
            rectangles = FaceRecognitionRectangle.objects.filter(deleted=None).annotate(
                number_of_suggestions=Count('face_recognition_suggestions')).order_by('-number_of_suggestions').values(
            ...
            if rectangles is not None:
                ...
                if all_rectangles is not None:
                    if album_id is not None:
                        next_rectangle = all_rectangles.filter(photo_id__in=album_photo_ids).order_by('?').first()
                    else:
    ...
    if not next_rectangle:
        if album_id is None:
            ...
            next_rectangle = FaceRecognitionRectangle.objects.filter(photo_id__in=album_photo_ids).order_by(
                '?').first()
    ...
    subject_id = rectangle and (rectangle.subject_consensus_id or rectangle.subject_ai_suggestion_id)
    has_consensus = bool(subject_id)
