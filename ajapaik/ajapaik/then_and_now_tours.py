...
def map_view(request, tour_id=None):
    if tour_id is None:
        return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.frontpage'))
    ...
    if tour.grouped and not user_has_group and tour.user != profile:
        return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.choose_group', args=(tour.pk,)))
    ret = {
        'tour': tour,
        'is_map': True
    }
...
def choose_group(request, tour_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.map_view', args=(tour.pk,)))
...
def detail(request, tour_id, photo_id, rephoto_id=None):
    ...
    if rephoto_id:
        ...
        if 'then_and_now_rephoto_open' in request.session and request.session['then_and_now_rephoto_open']:
            ...
            if rp:
                return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.detail', args=(tour.pk, photo.pk, rp.pk)))
    profile = request.user.profile
        ...
        if tour.photo_set_type == tour.FIXED:
            for each in tour.photos.all():
                if not TourRephoto.objects.filter(tour=tour, original=each, user=profile).exists():
                    ...
                    break
            if each_photo_done:
                return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.tour_complete', args=(tour.pk,)))
        return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.map_view', args=(tour.pk,)))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if new_tour.photo_set_type == Tour.OPEN:
                return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.create_tour_step_3', args=(new_tour.pk,)))
            else:
                return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.create_tour_step_2', args=(new_tour.pk,)))
...
def create_tour_step_2(request, tour_id):
    ...
    if not tour.user == profile:
        return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.create_tour_step_1'))
    if tour.photo_set_type == Tour.OPEN:
        return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.create_tour_step_3'))
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if tour:
                ...
                for each in tour.photos.all():
                    ...
                    i += 1
            return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.create_tour_step_3', args=(tour.pk,)))
        else:
            ret['form'] = form
...
def create_tour_step_3(request, tour_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.create_tour_step_4', args=(tour.pk,)))
...
def create_tour_step_4(request, tour_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and group_formset.is_valid():
            group_formset.save()
            form.save()
            return redirect(reverse('ajapaik.ajapaik.then_and_now_tours.create_tour_step_5', args=(tour.pk,)))
