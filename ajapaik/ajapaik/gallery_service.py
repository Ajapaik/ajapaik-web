...
def get_filtered_data_for_gallery(
    ...
    if page:
        ...
        # and preserve the order of photo_ids
        preserved_order = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(photo_ids[start:end])])
        photos = photos.filter(id__in=photo_ids[start:end]).order_by(preserved_order)
    else:
