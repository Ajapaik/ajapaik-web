...
class Login(APIView):
    ...
    def post(self, request):
        ...
        if login_type == forms.APILoginForm.LOGIN_TYPE_AJAPAIK:
            ...
            user = self._authenticate_with_google(request._request, username)
...
@never_cache
@permission_classes((AllowAny,))
def api_album_thumb(album_id, thumb_size=250):
    ...
    try:
        ...
        im = urlopen('https://rlv.zcache.com/doge_sticker-raf4b2dbd11ec4a7992e8bf94601ace75_v9wth_8byvr_512.jpg')
    content = im.read()
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if hasattr(image, '_getexif'):  # Present only in JPEGs.
                exif = image.getexif() or {}  # Returns None if no EXIF data.
...
class APIUserMe(AjapaikAPIView):
    def _handle_request(self, data, user, request):
        content = {
            'error': 0,
    ...
    def _handle_request(self, data, user, request):
        ...
        if form.is_valid():
            if user.is_authenticated:
                ...
                if lat and lon:
                    ref = Point(x=lon, y=lat, srid=4326)
                    photos = photos.annotate(distance=Distance('geography', ref)).order_by('distance')[start:end]
                else:
...
class PhotoAppliedOperations(AjapaikAPIView):
    ...
    def post(self, request):
        ...
        for photo_id in photo_ids:
            ...
            original_rotation = photo.rotated or 0
