...
class Album(Model):
    ...
    def get_all_photos_queryset_with_subalbums(self):
        historic_photo_ids = self.get_historic_photos_queryset_with_subalbums().values('id').order_by()
        return Photo.objects.filter(
            Q(id__in=historic_photo_ids) | Q(rephoto_of__in=historic_photo_ids)
        ).distinct()

    def get_comment_count_with_subalbums(self):
        historic_photo_ids = self.get_historic_photos_queryset_with_subalbums().values('id').order_by()
        # Sum comments on historic photos
        historic_sum = Photo.objects.filter(id__in=historic_photo_ids, comment_count__gt=0).aggregate(
            Sum('comment_count'))['comment_count__sum'] or 0
        # Sum comments on rephotos of those photos
        rephoto_sum = Photo.objects.filter(rephoto_of__in=historic_photo_ids, comment_count__gt=0).aggregate(
            Sum('comment_count'))['comment_count__sum'] or 0
    ...
    def set_calculated_fields(self):
        ...
        self.geotagged_photo_count_with_subalbums = historic_photo_qs.filter(lat__isnull=False,
                                                                             lon__isnull=False).count()

        ### Finally calculate rephoto count
        self.rephoto_count_with_subalbums = Photo.objects.filter(
            rephoto_of__in=historic_photo_qs.values('id')).count()

        # Comment count
        self.comments_count_with_subalbums = self.get_comment_count_with_subalbums()

        # Similar photos and confirmed similar photos count
        imagesimilarity_qs = ImageSimilarity.objects.filter(
            from_photo__in=self.get_all_photos_queryset_with_subalbums().values('id')).only('pk').distinct(
            'pk').order_by()
...
class Photo(Model):
    ...
    def add_to_source_album(self, update_stats=True):
