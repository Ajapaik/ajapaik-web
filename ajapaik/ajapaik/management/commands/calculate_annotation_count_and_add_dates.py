...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for photo in photos:
            try:
                facerecognition_rectangles = FaceRecognitionRectangle.objects.all().filter(photo__id=photo.id)
                object_annotations = ObjectDetectionAnnotation.objects.all().filter(photo__id=photo.id)
                first = None
                latest = None
                if facerecognition_rectangles.exists():
                    first = facerecognition_rectangles.order_by('created').first().created
                    latest = facerecognition_rectangles.order_by('-modified').first().modified
                ...
                photo.annotation_count = facerecognition_rectangles.count() + object_annotations.count()
                photo.light_save()
            except Exception:
                continue
