...
from ajapaik.ajapaik.views import Photo, Album, Profile, _get_album_choices
...
class Command(BaseCommand):
    ...
    def _get_album_choices(self, qs=None, start=None, end=None):
        # TODO: Sort out
        ...
        if qs != None and qs.exists():
            ...
            print(albums.query)
#            t=list(albums)
            print(str(counter) + " c01: " + str((time.time() - starttime2))) 
            starttime2 = time.time()

        else:
            albums = Album.objects.filter(is_public=True).prefetch_related('cover_photo').order_by('-created')#[start:end]
#            t=list(albums)
            print(str(counter) + " c02: " + str((time.time() - starttime2))) 
            starttime2 = time.time()

        albums=albums.filter(id__in=list(albums.values_list('id', flat=True)[start:end]) )
...
#            print(a.cover_photo.id)
#            p=Photo.objects.get(id=337519)
#            print(str(p.image))
            break

        print(str(counter) + " c03: " + str((time.time() - starttime2))) 
        starttime2 = time.time()

        return albums

    def _get_async_album(self, album_id=None, page_override=None):
        counter=0
        starttime2 = time.time()
        ...
        albums = self._get_album_choices(None, start, end)
        print(str(counter) + " b00: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
...
#        print("album_photo_ids: " + str(len(album_photo_ids)))
        ...
        if q and show_photos:
            ...
            photos = photos.filter(pk__in=[r.pk for r in sqs[:1000]], rephoto_of__isnull=True)


        counter=counter+1
        print(str(counter) + " b1b: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
        ...
        if not album:
            print(Photo.objects.filter(pk__in=photos).only('id').query)
#            albumsize_before_sorting=Photo.objects.filter(pk__in=photos).count()
            albumsize_before_sorting=photos.count()
            print(albumsize_before_sorting)

        counter=counter+1
        print(str(counter) + " b1: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
        ...
        if order1 == 'time':
            if order2 == 'rephotos':
                ...
                counter=counter+1
                print(str(counter) + " b1c: " + str((time.time() - starttime2))) 
                starttime2 = time.time()
        ...
        counter=counter+1
        print(str(counter) + " a2: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
        ...
        counter=counter+1
        print(str(counter) + " a3: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
    ...
    def _get_filtered_data_for_frontpage2(self, album_id=None, page_override=None):
        print("_get_filtered_data_for_frontpage2")
        starttime = time.time()
        starttime2 = time.time()
...
#        print("album_photo_ids: " + str(len(album_photo_ids)))
        ...
        counter=counter+1
        print(str(counter) + " a1c: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
        ...
        if album:
            ...
            for sa in album.subalbums.exclude(atype=Album.AUTO):
                sa_ids.append(sa.id)
            photos = photos.filter(albums__in = sa_ids).distinct()

        counter=counter+1
        print(str(counter) + " a1d: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
        ...
        counter=counter+1
        print(str(counter) + " a1e: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
        ...
        if order1 == 'closest' and lat and lon:
            ...
            if order2 == 'rephotos':
                ...
                photos_with_comments = photos.filter(first_comment__isnull=False).count()
            elif order2 == 'geotags':
                if order3 == 'reverse':
        ...
        if 0 and q and show_photos:
            print("Query: " + str(q))
            sqs = SearchQuerySet().models(Photo).filter(content=AutoQuery(q))

            counter=counter+1
            print(str(counter) + " sqs4a: " + str((time.time() - starttime2))) 
            starttime2 = time.time()

            photos = photos.filter(pk__in=[r.pk for r in sqs], rephoto_of__isnull=True)

            counter=counter+1
            print(str(counter) + " sqs4b: " + str((time.time() - starttime2))) 
            starttime2 = time.time()

        if q and show_photos:
            print("Query: " + str(q))
            photos_ids = list(photos.values_list('id', flat=True))
            photos_in = Photo.objects.filter(id__in=photos_ids)
            sqs = RelatedSearchQuerySet().models(Photo).load_all()
            sqs = sqs.filter(content=AutoQuery(q))
            sqs = sqs.load_all_queryset(Photo, photos_in)

            counter=counter+1
            print(sqs.count())
            print(str(counter) + " sqs1c: " + str((time.time() - starttime2))) 
            starttime2 = time.time()

            sqs_ids = list(sqs)

            print(len(list(sqs_ids)))
            counter=counter+1
            print(str(counter) + " sqs2c: " + str((time.time() - starttime2))) 
            starttime2 = time.time()
...
#            photos = photos.filter(pk__in=[r.pk for r in sqs], rephoto_of__isnull=True)
            ...
            counter=counter+1
            print(str(counter) + " sqs3c: " + str((time.time() - starttime2))) 
            starttime2 = time.time()
...
#            start, end, total, max_page, page = get_pagination_parameters(page, page_size, photos.order_by().count())
            # limit QuerySet to selected photos so it is faster to evaluate in next steps
#            photos=photos.filter(id__in=photos[start:end])
#            print(len(photos_ids))
#            li1 = list(photos_ids[start:end])
#            print(li1)
            photos=photos.filter(id__in=photos_ids[start:end])
#            li2 = list(photos.values_list('id', flat=True))
#            print(list(set(li1) - set(li2)) + list(set(li2) - set(li1)))


#            print("Albumsize:" +  str(albumsize))
        counter=counter+1
        print(str(counter) + " a1f1: " + str((time.time() - starttime2))) 
        starttime2 = time.time()

#        photos=photos.filter(id__in=photos[start:end])

        counter=counter+1
        print(str(counter) + " a1f2: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
...
#        print(len(photos_ids))
        ...
        counter=counter+1
        print(str(counter) + " a1g: " + str((time.time() - starttime2))) 
        starttime2 = time.time()
