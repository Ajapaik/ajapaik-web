...
class PhotoSerializer(PhotoRepresentationSerializer):
    ...
    def get_favorited(self, instance: Photo):
        ...
        if request and request.user.is_authenticated:
            return instance.likes.filter(profile=request.user.profile).exists()
        return False
    ...
    def get_in_selection(self, instance):
        request = self.context.get('request')
        if request:
            selection = request.session.get('photo_selection', {})
            return str(instance.id) in selection
        return False
...
class PhotoDetailsSerializer(PhotoRepresentationSerializer):
    ...
    def get_user_likes(self, instance):
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            return instance.likes.filter(profile=request.user.profile, level=1).exists()
        return False

    def get_user_loves(self, instance):
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            return instance.likes.filter(profile=request.user.profile, level=2).exists()
        return False
...
class PhotoWithDistanceSerializer(PhotoSerializer):
    ...
    class Meta(PhotoSerializer.Meta):
        fields = (
            ...
            'favorited', 'in_selection', 'comment_count'
        )
