(function() {
    ...
    var AjapaikGeotagger = function(node, options) {
        ...
        this.windowResizeListenerFunction = function() {
            if (
                that.options.markerLocked &&
                !that.fullScreenActive &&
                !that.feedbackMode &&
                !that.drawAzimuthLineOnMouseMove
            ) {
        ...
        this.mapMousemoveListenerFunction = function(e) {
            if (!that.feedbackMode && that.firstMoveDone) {
                if (that.drawAzimuthLineOnMouseMove) {
                    that.angleBetweenMouseAndMarker = that.radiansToDegrees(
                        that.getAzimuthBetweenMouseAndMarker(e),
                    );
                    that.azimuthLine.icons[0].repeat = '7px';
                    that.panoramaMarker.setVisible(false);
                    that.azimuthLine.setPath([
                        that.realMarker.position,
                        that.simpleCalculateMapLineEndPoint(
                            that.angleBetweenMouseAndMarker,
                            that.realMarker.position,
                            0.02,
                        ),
                    ]);
            ...
            if (!that.feedbackMode) {
                if (that.mapMarkerDragListenerActive) {
                    if (
                        ...
                        that.angleBetweenMarkerAndPanoramaMarker = that.radiansToDegrees(
                            that.getAzimuthBetweenTwoMarkers(
                                that.realMarker,
                                that.panoramaMarker,
                            ),
                        );
        ...
        this.wheelFunctionFF = function(e) {
            if (
                that.customFFWheelFunctionActive &&
                !that.streetPanorama.getVisible()
            ) {
        ...
        this.wheelFunctionNonFF = function(e) {
            if (
                that.customNonFFWheelFunctionActive &&
                !that.streetPanorama.getVisible()
            ) {
                var now = new Date().getTime();
    ...
    AjapaikGeotagger.prototype = {
        ...
        initializeMap: function() {
            ...
            this.vgmapi = new VanalinnadGooglemApi(null, true, this.map);
            ...
            this.lockButton
                .attr('title', gettext('Toggle map center lock'))
                .click(function() {
                    if (that.options.mode === 'vantage') {
                        var $this = $(this);
            ...
            this.mapShowSearchButton.click(function() {
                ...
                if (
                    typeof window.reportGeotaggerShowSearch === 'function' &&
                    !box.hasClass('d-none')
                ) {
            ...
            modeSelectionButtonGroup
                .find('#ajp-geotagger-approximate-mode-button')
                .attr('title', gettext('Approximate'))
                .html(gettext('Approximate'))
                .click(function() {
                    var $this = $(this);
            ...
            modeSelectionButtonGroup
                .find('#ajp-geotagger-vantage-point-mode-button')
                .attr('title', gettext('Vantage point'))
                .html(gettext('Vantage point'))
                .click(function() {
                    var $this = $(this);
            ...
            modeSelectionButtonGroup
                .find('#ajp-geotagger-object-mode-button')
                .attr('title', gettext('Object'))
                .html(gettext('Object'))
                .click(function() {
                    var $this = $(this);
            ...
            this.mapInstructions
                ...
                .text(
                    gettext(
                        'Grab and drag the MAP so that the marker is where the photographer was standing.',
                    ),
                );
            searchBox
                .attr('placeholder', gettext('Search box'))
            ...
            $('#ajp-geotagger-feedback')
                .find('#ajp-geotagger-feedback-thanks')
                .html(gettext('Thank you!'));
            this.map.controls[google.maps.ControlPosition.TOP_CENTER].push(
                searchBox.get(0),
            );
            this.map.controls[google.maps.ControlPosition.BOTTOM_CENTER].push(
                this.mapInstructions.get(0),
            );
            this.map.controls[google.maps.ControlPosition.BOTTOM_RIGHT].push(
                this.mapOpenInstructionsButton.get(0),
            );
            this.map.controls[google.maps.ControlPosition.TOP_RIGHT].push(
                this.mapShowSearchButton.get(0),
            );
            this.streetPanorama.controls[
                google.maps.ControlPosition.BOTTOM_RIGHT
                ].push(this.streetPanoramaExtraCloseButton.get(0));
            this.mapTypeChangedListener = google.maps.event.addListener(
                this.map,
                'maptypeid_changed',
                function() {
                    if (that.map.getMapTypeId() === 'old-maps') {
            ...
            google.maps.event.addListener(
                ...
                function() {
                    if (that.streetPanorama.getVisible()) {
                        ...
                        if (
                            typeof window.reportGeotaggerStreetPanoramaOpen === 'function'
                        ) {
                            window.reportGeotaggerStreetPanoramaOpen(
                                that.options.currentPhotoId,
                            );
                        }
                    } else {
                        if (that.streetViewAzimuth) {
                            var fauxClickPoint = that.simpleCalculateMapLineEndPoint(
                                that.streetViewAzimuth,
                                that.map.getCenter(),
                                0.001,
                            );
                            if (!that.feedbackMode) {
            ...
            $('<div/>')
                .attr('id', 'ajp-geotagger-suggestion-marker')
                .addClass('vantage')
                .appendTo(this.map.getDiv());
            ...
            feedbackNextButton.click(function() {
                ...
                if (
                    typeof window.reportGeotaggerSendFeedback === 'function' &&
                    feedbackVal
                ) {
            ...
            this.mapInner.addEventListener(
                'mousewheel',
                this.wheelFunctionNonFF,
                true,
            );
            this.mapInner.addEventListener(
                'DOMMouseScroll',
                this.wheelFunctionFF,
                true,
            );
            ...
            $('#ajp-geotagger-feedback-difficulty-prompt').html(
                gettext('How much has the depicted location changed?'),
            );
            ...
            this.map.controls[google.maps.ControlPosition.LEFT_CENTER].push(
                this.lockButton.get(0),
            );
            this.map.controls[google.maps.ControlPosition.TOP_RIGHT].push(
                modeSelectionButtonGroup.get(0),
            );
            google.maps.event.addListener(
                this.map,
                'mousemove',
                this.mapMousemoveListenerFunction,
            );
            google.maps.event.addListener(
                this.map,
                'dragstart',
                this.mapDragstartListenerFunction,
            );
            google.maps.event.addListener(
                this.map,
                'dragend',
                this.mapDragendListenerFunction,
            );
            google.maps.event.addListener(
                this.map,
                'center_changed',
                this.mapCenterChangedListener,
            );
            google.maps.event.addListener(
                this.map,
                'idle',
                this.mapIdleListenerFunction,
            );
            google.maps.event.addListener(
                this.map,
                'click',
                this.mapClickListenerFunction,
            );
            google.maps.event.addListener(
                this.realMarker,
                'drag',
                this.mapMarkerDragListenerFunction,
            );
            google.maps.event.addListener(
                this.realMarker,
                'dragend',
                this.mapMarkerDragendListenerFunction,
            );
        },
        // FIXME: Getting less and less DRY
        switchToApproximateMode: function() {
            $('#ajp-geotagger-suggestion-marker')
                .removeClass('vantage object')
                .addClass('approximate');
        ...
        switchToVantagePointMode: function() {
            $('#ajp-geotagger-suggestion-marker')
                .removeClass('approximate object')
                .addClass('vantage');
        ...
        switchToObjectMode: function() {
            $('#ajp-geotagger-suggestion-marker')
                .removeClass('vantage approximate')
                .addClass('object');
            this.lockButton.hide();
            this.lockMapToCenter();
            this.panoramaMarker.setVisible(false);
        ...
        initializeGeotaggerState: function(options) {
            ...
            if (
                ...
                options.currentPhotoId &&
                window.previouslyEditedPhotoIds.includes(options.currentPhotoId)
            ) {
                imageSrc += '?timestamp=' + Date.now();
            }
            this.geotaggerImageThumb
                .attr('src', imageSrc)
                .removeClass('ajp-photo-flipped');
            $('#ajp-geotagger-full-screen-image').removeClass('ajp-photo-flipped');
            this.description.html(options.description).addClass('d-none');
            if (options.sourceURL && (options.sourceName || options.sourceKey)) {
                this.source
                    .addClass('d-none')
                    .find('a')
                    .attr('title', options.sourceName + ' ' + options.sourceKey)
                    .attr('href', options.sourceURL)
                    .html(options.sourceName + ' ' + options.sourceKey);
            }
            ...
            $('#ajp-geotagger-map-instruction-text')
                .find('p')
                .text(
                    gettext(
                        'Grab and drag the MAP so that the marker is where the photographer was standing.',
        ...
        setCursorToPanorama: function() {
            this.map.setOptions({
                draggableCursor:
                    'url(/static/images/material-design-icons/ajapaik_custom_size_panorama.svg) 18 18, auto',
                draggingCursor:
                    'url(/static/images/material-design-icons/ajapaik_custom_size_panorama.svg) 18 18, auto',
            });
        },
            ...
            if (this.options.mode === 'vantage') {
                if (this.options.markerLocked) {
                    if (this.saveAzimuth) {
                        element.text(
                            gettext(
                                'Click the green button to save both the location and the direction of the view. Click on the map to unlock the direction, drag map to correct the location.',
                            ),
                        );
                    } else {
                        if (this.firstMoveDone) {
                            element.text(
                                gettext(
                                    'Click on the map to lock the direction of the view or click yellow button to save only the location.',
                                ),
                            );
                        } else {
                            element.text(
                    ...
                    if (this.saveAzimuth) {
                        element.text(
                            gettext(
                                'Click the green button to save both the location and the direction of the view. Click on the map to unlock the direction, drag MARKER to correct the location.',
                            ),
                        );
                        ...
                        element.text(
                            gettext(
                                'Now you can drag the MARKER where the photographer was standing and the MAP separately.',
                            ),
                        );
                    }
                }
                ...
                element.text(
                    gettext(
                        'Drag the map so that the circle covers the approximate location, click save when ready.',
                    ),
                );
            } else {
                element.text(
                    gettext(
                        'Drag the map so that the marker is on top of the object you see on the picture, click save when ready.',
                    ),
                );
            }
            google.maps.event.trigger(this.map, 'resize');
        ...
        fitSuggestionPhotosToContainers: function() {
            ...
            if (window.matchMedia('(min-width: 992px)').matches) {
                ...
                targetParent
                    .css('height', geotaggerPanel.height() + 'px')
                    .css('width', '50%');
        ...
        showInstructions: function() {
            docCookies.setItem(
                'ajapaik_closed_geotagger_instructions',
                false,
        ...
        saveGeotag: function() {
            ...
            if (that.saveAzimuth) {
                ...
                if (
                    typeof window.reportGeotaggerSaveLocationAndDirection === 'function'
                ) {
                    window.reportGeotaggerSaveLocationAndDirection(
                        that.options.currentPhotoId,
                    );
                }
            } else {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#ajp-geotagger-feedback-points').html(
                        gettext('Points awarded') + ': ' + response.current_score,
                    );
                    if (response.was_flip_successful === false) {
                        $('#ajp-geotagger-flip-feedback-title').html(
                            gettext('Picture flip feedback') + ':',
                        );
                        $('#ajp-geotagger-flip-feedback-text').html(response.flip_response);
                    }
                    if (response.was_flip_successful !== null) {
                        let photoUrl =
                            $('#ajp-geotagger-image-thumb').attr('src') +
                            '?timestamp=' +
                            Date.now();
                        $('#ajp-geotagger-image-thumb')
                            .attr('src', photoUrl)
                            .removeClass('ajp-photo-flipped');
                        that.options.fullScreenSrc += '?timestamp=' + Date.now();
                        $('#ajp-geotagger-full-screen-image').removeClass(
                            'ajp-photo-flipped',
                        );
                    }
                    ...
                    $('#ajp-geotagger-game-buttons').show();
                    if (
                        response.current_score > 0 &&
                        typeof window.reportGeotaggerCorrect === 'function'
                    ) {
                    ...
                    if (
                        parseInt(
                            response.current_score === 0 &&
                            typeof window.reportGeotaggerIncorrect === 'function',
                        )
                    ) {
                        window.reportGeotaggerIncorrect();
                    }
                    if (
                        response.new_geotag_count === 1 &&
                        response.estimated_location[0] &&
                        response.estimated_location[1]
                    ) {
                        window.reportGeotaggerNewlyMappedPhoto();
                    }
                    if (that.photoFlipped) {
                        if (
                            window.previouslyEditedPhotoIds.indexOf(
                                that.options.currentPhotoId,
                            ) === -1
                        ) {
                    ...
                    localStorage.setItem(
                        'previouslyEditedPhotoIds',
                        JSON.stringify(window.previouslyEditedPhotoIds),
                    );
                },
                error: function() {
                    $('#ajp-geotagger-game-buttons').show();
        ...
        displayFeedbackHeatmap: function(responseData) {
            ...
            for (i = 0; i < responseData.heatmap_points.length; i += 1) {
                newLatLng = new google.maps.LatLng(
                    responseData.heatmap_points[i][0],
                    responseData.heatmap_points[i][1],
                );
                heatmapPoints.push(newLatLng);
                latLngBounds.extend(newLatLng);
            }
            if (
                responseData.estimated_location &&
                responseData.estimated_location[0] &&
