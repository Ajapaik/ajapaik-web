...
function getDetectedObjectModifyRectangle(
  rectangleId,
  savedRectangle,
  configuration
) {
...
function getDetectedFaceModifyRectangle(
  rectangleId,
  savedRectangle,
  configuration
) {
...
function getScaledRectangleConfiguration(
  scaledRectangle,
  annotation,
  imageAreaDimensions
) {
...
function getSavedAnnotation(scaledRectangle, annotation, imageAreaDimensions) {
  let configuration = getScaledRectangleConfiguration(
    scaledRectangle,
    annotation,
    imageAreaDimensions
  );
...
function getCurrentPhotoDimensionScalesInRelationToTheOriginalPhoto(
  imageAreaDimensions
) {
...
function scaleRectangleForCurrentImageSize(
  scalingParametersForCurrentImageSize,
  savedRectangle
) {
...
function drawAnnotations(detections, imageAreaDimensions) {
  ...
  setTimeout(function () {
    ...
    let scalesInRelationToTheOriginalPhoto =
      getCurrentPhotoDimensionScalesInRelationToTheOriginalPhoto(
        imageAreaDimensions
      );

    detections.forEach(function (savedRectangle) {
      if (savedRectangle.id !== null) {
        let scaledRectangle = scaleRectangleForCurrentImageSize(
          scalesInRelationToTheOriginalPhoto,
          savedRectangle
        );
        let annotation = getSavedAnnotation(
          scaledRectangle,
          savedRectangle,
          imageAreaDimensions
        );
...
function centerAnnotationContainerInRegardsToBlackBorders(
  annotationContainer,
  imageContainer
) {
  let imageWidthToHeightRelation =
    window.currentPhotoOriginalWidth / window.currentPhotoOriginalHeight;
  let isHeightInFullLength = isPlacingBlackBordersOnSides(
    imageWidthToHeightRelation
  );
...
function drawAnnotationContainer(imageContainer, isRedraw) {
  ...
  centerAnnotationContainerInRegardsToBlackBorders(
    annotationContainer,
    imageContainer
  );
  ...
  annotationContainer.attr(
    'id',
    constants.elements.ANNOTATION_CONTAINER_ID_ON_IMAGE
  );
