(function($) {
    ...
    var AjapaikDater = function(node, options) {
        ...
        this.hasGoodDashesOrDots = function(input) {
            const dashCount = (input.raw.match(/-/g) || []).length,
        ...
        this.cleanAndValidate = function() {
            let userInput = that.extractUserInput(
                that.$UI.find('input').val().replace(/ /g, ''),
            );
            that.invalid = false;
            if (that.hasGoodDashesOrDots(userInput)) {
                ...
                if (
                    userInput.toLocalizationFormat ||
                    userInput.fromLocalizationFormat
                ) {
            ...
            if (data.isRange) {
                if (data.from && data.to) {
                    if (data.fromApproximate && data.toApproximate) {
                        fmt = gettext(
                            'Between approximately %(from)s and approximately %(to)s',
                        );
                        feedbackStr = interpolate(
                            fmt,
                            {
                                from: data.from
                                    .locale(currentLocale)
                                    .format(data.fromLocalizationFormat),
                                to: data.to
                                    .locale(currentLocale)
                                    .format(data.toLocalizationFormat),
                            },
                            true,
                        );
        ...
        this.giveFeedback = function() {
            ...
            const formGroup = that.$UI.find('.form-group'),
        ...
        this.submitConfirmation = function(id) {
            ...
            $.ajax({
                ...
                error: function() {
                    ...
                    $('#ajp-dater-feedback').html(
                        gettext('Server received invalid data.'),
                    );
                },
            });
        };
        this.submit = function() {
            ...
            if (!that.invalid) {
                ...
                if (
                    payload.comment &&
                    typeof window.reportDaterSubmitWithComment === 'function'
                ) {
        ...
        this.buildPreviousDatingsDiv = function(datings) {
            ...
            $.each(datings, function(k, v) {
                ...
                parsedInput = that.getValidDates(
                    that.calculateDateFormats(
                        that.extractApproximates(that.extractUserInput(v.raw)),
                    ),
                );
                let userLink = v.profile_id
                    ? '<a href="/user/' + v.profile_id + '">' + userStr + '</a>:'
                    : '<b>' + gettext('MUIS') + '</b>';
                previousDatings.append(
                    userLink +
                    ' ' +
                    that.generateDateString(parsedInput) +
                    ' ' +
                    commentStr +
                    '<span class="badge" style="left:0;">' +
                    v.confirmation_count +
                    '</span><span onclick="window.confirmDating(' +
                    v.id +
                    ')" class="material-icons notranslate ajp-dater-confirm-button' +
                    addClass +
                    '" data-id="' +
                    v.id +
                    '" title="' +
                    gettext('Confirm dating') +
                    '">thumb_up</span></div><br>',
                );
            });
        };
        this.giveDatingSubmittedFeedback = function(confirmation) {
            ...
            that.$UI
                .find('#ajp-dater-feedback-card')
                .html('<h1>' + gettext('Thanks!') + '</h1><p>' + feedbackStr + '</p>')
                .show();
        };
        this.$UI = $(this.node);
    ...
    AjapaikDater.prototype = {
        ...
        initializeDater: function() {
            ...
            that.$UI
                .find('input')
                .attr('placeholder', gettext('YYYY.MM.DD'))
            ...
            that.$UI
                .find('#ajp-dater-cancel-button')
                .attr('title', gettext('Cancel'))
                .click(function(e) {
                    e.preventDefault();
            ...
            that.$UI
                .find('#ajp-dater-submit-button')
                .attr('title', gettext('Submit'))
                .click(function(e) {
                    e.preventDefault();
                    that.submit();
                });
            that.$UI
                .find('#ajp-dater-comment')
                .on('focus', function() {
                    window.datingFocused = true;
                })
                .on('blur', function() {
            ...
            that.$UI.find('#ajp-dater-toggle-comment-button').click(function(e) {
                e.preventDefault();
                const target = that.$UI.find('#ajp-dater-comment');
                target.toggleClass('d-none');
                if (
                    !target.hasClass('d-none') &&
                    typeof window.reportDaterOpenComment === 'function'
                ) {
                    window.reportDaterOpenComment();
                }
            });
            that.$UI
                .find('#ajp-dater-tutorial-card #tutorial-block')
                .html(
                    '<ul><li>' +
                    gettext('Use YYYY.MM.DD format (MM.DD not obligatory)') +
                    ':</br><span class="font-italic">' +
                    gettext('1878 | 1902.02') +
                    '</span></li><li>' +
                    gettext(
                        'Mark date ranges or before/after with either "-" or ".."',
                    ) +
                    ':</br><span class="font-italic">' +
                    gettext('1910-1920 | 1978.05.20..1978.06.27 | -1920 | 1935..') +
                    '</span></li><li>' +
                    gettext('Approximate date in brackets') +
                    ':</br><span class="font-italic">' +
                    gettext('(1944) | (1940.05)..1941.08.21)') +
                    '</span></li></ul>',
                );
            ...
            that.$UI.find('#ajp-dater-close-tutorial-button').click(function() {
                ...
                that.$UI.find('#ajp-dater-open-tutorial-button').show();
                docCookies.setItem(
                    'ajapaik_closed_dater_instructions',
                    true,
            ...
            that.$UI
                .find('#ajp-dater-anonymous-user-card')
                .find('div')
                .html(gettext('You\'re anonymous'));
            that.$UI
                .find('#ajp-dater-anonymous-user-card')
                .find('span')
                .click(function() {
