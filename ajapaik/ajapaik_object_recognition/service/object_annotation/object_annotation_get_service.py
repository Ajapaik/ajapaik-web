...
def get_all_annotations(user_id, photo_id=None):
    ...
    album_ids = AlbumPhoto.objects.filter(photo=photo).exclude(album_id__in=person_subject_ai_suggestion_ids) \
        .exclude(album_id__in=person_subject_consensus_ids).values_list('album_id', flat=True)
    person_albums = Album.objects \
        .filter(id__in=album_ids, atype=Album.PERSON)
...
def map_person_album_to_rectangle(album: Album, user_id: int, photo_id: int):
    albumphoto = AlbumPhoto.objects.filter(album=album, photo_id=photo_id).first()

    if albumphoto is None:
        user = None
    else:
    ...
    return DetectionRectangle({
        ...
        'y2': None,
        'subject_id': album.id,
        'subject_name': album.name,
        'is_editable': False,
        'is_added_by_current_user': False,
        'gender': object_annotation_utils.parse_gender_to_constant(album.gender),
        'age': None,
