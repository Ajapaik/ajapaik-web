...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        results.write('id\temail\tfb_name\tfb_email\tgoogle_name\tgoogle_email\tscore\ttrustworthiness\tfirst_geotag\tlatest_geotag\tgeotag_count\tfirst_rephoto\tlatest_rephoto\trephoto_count\tfirst_curation\tlatest_curation\tcuration_count\tfirst_recuration\tlatest_recuration\trecuration_count\tfb_comment_count\tfavorite_count\n')
        for p in profiles:
            ...
            if p.user.email and not p.user.email == ' ':
                email = p.user.email
            results.write('\t'.join([str(p.id), email, p.fb_name, p.fb_email, p.google_plus_name, p.google_plus_email, str(p.score), str(trustworthiness),
