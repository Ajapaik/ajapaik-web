...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        profiles = Profile.objects.filter(Q(score__gt=0) | Q(fb_name__isnull=False) | Q(google_plus_name__isnull=False))\
            .distinct('user_id')
        results = codecs.open(ABSOLUTE_PROJECT_ROOT + '/project/ajapaik/management/commands/results/results.txt', 'w', 'utf-8')
        results.write('id\tfb_name\tfb_email\tgoogle_name\tgoogle_email\tscore\ttrustworthiness\tfirst_geotag\tlatest_geotag\tgeotag_count\tfirst_rephoto\tlatest_rephoto\trephoto_count\tfirst_curation\tlatest_curation\tcuration_count\tfirst_recuration\tlatest_recuration\trecuration_count\tfb_comment_count\tfavorite_count\n')
        for p in profiles:
            ...
            results.write('\t'.join([str(p.id), p.fb_name, p.fb_email, p.google_plus_name, p.google_plus_email, str(p.score), str(trustworthiness),
