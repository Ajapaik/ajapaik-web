...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        cursor.execute('''INSERT INTO project_geotag (id, lat, lon, type, created, modified, user_id, photo_id, is_correct, score, trustworthiness, azimuth, zoom_level, azimuth_score) SELECT gt.id, gt.lat, gt.lon, gt.type, gt.created, gt.modified, gt.user_id, gt.photo_id, gt.is_correct, gt.score, gt.trustworthiness, gt.azimuth, gt.zoom_level, gt.azimuth_score FROM dblink('dbname=rephoto_backup user=rephoto password=reph0t0sqluser', 'SELECT id, lat, lon, type, created, modified, user_id, photo_id, is_correct, score, trustworthiness, azimuth, zoom_level, azimuth_score FROM project_geotag')
