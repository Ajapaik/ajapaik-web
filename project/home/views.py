...
def game(request):
    ...
    ret["album"] = album
    if album:
        ret["facebook_share_photos"] = album.photos.values_list('id', flat=True)[:5]
    elif area:
        ret["facebook_share_photos"] = Photo.objects.filter(area=area, rephoto_of__isnull=True).order_by("?").values_list('id', flat=True)[:5]
