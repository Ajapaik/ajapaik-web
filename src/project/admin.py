...
class PhotoAdmin(AdminImageMixin, admin.ModelAdmin):
    ...
    def _distance_between_two_points_on_sphere(lon_1, lat_1, lon_2, lat_2):
        ...
        lon_2_rad = lon_2 * rad
        lat_2_rad = lat_2 * rad
        cos_angle = math.sin(lat_1_rad) * math.sin(lat_2_rad) + math.cos(lat_1_rad) * math.cos(lat_2_rad) * math.cos(lon_2_rad - lon_1_rad)
    ...
    def save_model(self, request, obj, form, change):
        if obj.lat and obj.lon and obj.bounding_circle_radius:
            ...
            for geo_tag in all_photo_geo_tags:
                if self._distance_between_two_points_on_sphere(obj.lon, obj.lat, geo_tag.lon, geo_tag.lat) > obj.bounding_circle_radius:
