...
def geotag_add(request):
	data = request.POST
	is_correct, current_score, total_score, leaderboard_update, location_is_unclear = get_next_photos_to_geotag.submit_guess(request.get_user().get_profile(), data['photo_id'], data.get('lon'), data.get('lat'), hint_used=data.get('hint_used'), azimuth=data.get('azimuth'), zoom_level=data.get('zoom_level'))
	return HttpResponse(json.dumps({
...
def fetch_stream(request):
	...
	data["photos"], data["user_seen_all"] = filters.get_filtered_qs().get_next_photos_to_geotag(
		request.get_user().get_profile().pk, 4, request.GET.get('extra'), request.GET.get("only_untagged"), request.GET.get("retry_old"))
